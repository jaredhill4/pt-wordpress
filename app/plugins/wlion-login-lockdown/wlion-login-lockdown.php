<?php
/*
Plugin Name: White Lion: Login LockDown
Plugin URI: https://www.wlion.com/
Description: A fork of the Login Lockdown plugin by Michael VanDeMar (http://www.bad-neighborhood.com/). Adds some extra security to WordPress by restricting the rate at which failed logins can be re-attempted from a given IP range. Distributed through <a href="http://www.bad-neighborhood.com/" target="_blank">Bad Neighborhood</a>.
Version: v1.8.1
Author: White Lion
...
function get_ip() {
	...
	if (array_key_exists('X-Forwarded-For', $headers) and filter_var($headers['X-Forwarded-For'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
		$the_ip = $headers['X-Forwarded-For'];
	} elseif (array_key_exists('HTTP_X_FORWARDED_FOR', $headers) and filter_var($headers['HTTP_X_FORWARDED_FOR'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
